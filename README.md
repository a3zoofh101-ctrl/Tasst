# بوست — منصة إدارة خدمات التسويق الرقمي
...
> ملاحظة: هذا المستودع يحتوي أيضًا على مشاريع تجريبية أخرى غير مرتبطة (متجر عطور `/`, موقع بلدية `/baladiya`, لعبة `/moqnaas`). منصة بوست تعيش تحت المسارات `/smm`, `/login`, `/register`, `/dashboard/*`, `/admin/*` ولا تتداخل معها.
...
`npm run build` يشغّل تلقائيًا: `prisma generate` → `prisma migrate deploy` → `prisma db seed` → `next build`. الـ Seed آمن للتكرار (Idempotent) — لا يُنشئ بيانات مكرّرة عند إعادة النشر.
...
### النشر بدون جهاز كمبيوتر (Vercel + Neon، من المتصفح فقط)

مناسب لو ما عندك إلا جوال:

1. أنشئ قاعدة بيانات مجانية على [neon.tech](https://neon.tech) (PostgreSQL سحابي) — انسخ رابط الاتصال (`DATABASE_URL`)
2. على [vercel.com](https://vercel.com) → **Add New Project** → اختر مستودع GitHub هذا (فرع `claude/smm-platform-arabic-t02s07` أو الفرع الذي دمجته)
3. في إعدادات المشروع → **Environment Variables** أضف كل متغيرات `.env.example` (على الأقل: `DATABASE_URL`, `AUTH_SECRET`, `ENCRYPTION_KEY`, `CRON_SECRET`, `ADMIN_EMAIL`, `ADMIN_PASSWORD`, `ADMIN_NAME`, `APP_URL` = رابط النشر بعد أول Deploy)
4. **لا تضف** `SEED_REAL_PROVIDER_*` هنا (قد يتسبب بطء كتالوج كبير بتجاوز مهلة البناء) — استخدم بدلًا منه زر **"استيراد كل الخدمات تلقائيًا"** من `/admin/providers` بعد النشر (يعمل من نفس المتصفح، بدون أي طرفية)
5. اضغط **Deploy** — Vercel يشغّل كل شيء تلقائيًا (Migration + Seed الأساسي)
6. افتح رابط النشر (`https://your-project.vercel.app/login`) من Safari وسجّل دخول بحساب الأدمن
...
طريقتان من `/admin/providers`:

- **"مزامنة الخدمات"** — يجلب كتالوج المزوّد ويحفظه كـ `ProviderService` فقط (لا يظهر للعملاء). بعدها من `/admin/services` تستورد كل خدمة يدويًا مع تحديد المنصة/التصنيف وهامش الربح — مناسب لو تريد التحكم الدقيق بكل خدمة على حدة.
- **"استيراد كل الخدمات تلقائيًا"** (لمزودي Generic فقط) — يجلب الكتالوج الكامل ويكتشف منصة كل خدمة تلقائيًا من اسمها، ويفعّلها **جميعًا فورًا** بهامش ربح واحد تحدده (0% = سعر التكلفة تمامًا). مناسب للاستخدام الشخصي أو عند وجود عدد كبير من الخدمات. يعمل بالكامل من المتصفح — لا يحتاج طرفية، ويصلح لإعادة المحاولة (خدمة مستوردة مسبقًا تُتجاوز تلقائيًا).

### ربط مزوّد حقيقي تلقائيًا عند أول تشغيل (اختياري، للتطوير المحلي)

لتجنّب الخطوات اليدوية أعلاه (مفيد للاستخدام الشخصي حيث تريد كل الخدمات مفعّلة فورًا بلا هامش ربح)، أضف في `.env`:

```
SEED_REAL_PROVIDER_NAME="اسم المزوّد"
SEED_REAL_PROVIDER_API_URL="https://provider.example.com/api/v2"
SEED_REAL_PROVIDER_API_KEY="مفتاحك"
SEED_REAL_PROVIDER_MARKUP_PERCENT=0
```

ثم `npx prisma db seed`. يقوم تلقائيًا بـ: إضافة المزوّد (مشفّر)، سحب رصيده وكتالوجه بالكامل، تصنيف كل خدمة تحت منصتها (يكتشف اسم المنصة من نص الخدمة/التصنيف تلقائيًا)، واستيراد **كل** الخدمات مفعّلة فورًا بالهامش المحدد. الأمر آمن لإعادة التشغيل (Idempotent) — لن يُنشئ تكرارات. إن تعذّر الوصول للمزوّد وقت التشغيل، يُحفظ المزوّد ويُطبع تحذير، وتكمل بقية الـ Seed (الأدمن، الحساب التجريبي...) بشكل طبيعي — أعد `npx prisma db seed` لاحقًا لإتمام المزامنة.
